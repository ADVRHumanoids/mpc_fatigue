...
If you set Check.RightConst = True the right robot joints 0-1-2-3-4 will be constrained.
    ...
    for k in range(N):
        ...
        if Check.TorqueConstraint == True:
                ...
                if Check.RightConst == True:
                    ...
                    g.append(vertcat(tauRR[5]))
                    lbg += np.full((1,1),-500.0)[0].tolist()   
                    ubg += np.full((1,1),500.0)[0].tolist()
                    ...
                    if k < int(N/3):
                        lbg += [-300.0] 
                        ubg += [400.0]
                        tau_RR3.append(-300.0)
                        tau_RR3.append(400.0)
                    elif int(N/3) <= k < int(2*N/3):
                        lbg += [-20.0] 
                        ubg += [20.0]
                        tau_RR3.append(-20.0)
                    ...
                    if k < int(N/3):
                        lbg += [-300.0] 
                        ubg += [400.0]
                        tau_RR4.append(-300.0)
                        tau_RR4.append(400.0)
                    elif int(N/3) <= k < int(2*N/3):
                        lbg += [-20.0] 
                        ubg += [20.0]
                        tau_RR4.append(-20.0)
        ...
        for j in range(nq): 
            ...
            if Check.OCP == True:
                if u == 2:
                    ...
                    elif j == 3 and tau_RR1 != []:
                        plt.plot(tgrid,tau_RR3[0::2],'--')
                        plt.plot(tgrid,tau_RR3[1::2],'--')
                    elif j == 4 and tau_RR2 != []:
                        plt.plot(tgrid,tau_RR4[0::2],'--')
                        plt.plot(tgrid,tau_RR4[1::2],'--')
                else:
                    if j == 0 and tau_LR0 != []:
