...
#    k4 = h *k3
#    
#    q_next = qc_k + h/6 *(k1 + 2*k2 + 2 *k3 +k4)
@author: user
"""
#FLAGS
plot = 1
...
plt.figure(0)
plt.clf()
plt.step(tgrid,lbg_torque,'-')
plt.step(tgrid,ubg_torque,'-')
plt.legend(['lbg_torque','ubg_torque'])
plt.title('Torque limits')
plt.grid()
plt.show()
...
m = 80; #box mass
pos_toll = 0.0005 #tollerance in distance between the two arm
...
#        ubw +=  qcdot_init
    ...
    g.append(tau_second[1])
    lbg +=  [lbg_torque[k]] #np.zeros(1).tolist()  
    ubg +=  [ubg_torque[k]] #np.zeros(1).tolist() 
    ...
    J += 100 * dot( p_box[2] - SX(0.5),p_box[2] - SX(0.5) )
