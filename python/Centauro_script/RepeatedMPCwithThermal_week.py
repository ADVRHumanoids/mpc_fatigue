        ...
        for k in range(N):
            ...
            if Check.Const1 == True or Check.Const3 == True or Check.Const4 == True : 
                ...
                if Check.Const1 == True or Check.Const3 == True:
                    ### DA COMPARARE ###
#                    mat_inv = vertcat(horzcat(R01.T,-mtimes(R01.T,pL)),SX([0.0,0.0,0.0,1.0]).T)
#                    #Compute the relative position of the E1 in E2
#                    pos_2in1_w1 = mtimes(mat_inv,vertcat(pR,SX(1.0)))
#                    #pos_1in2_w1 = mtimes(mat_inv,vertcat(pL,SX(1.0)))
#                    #pos_1in2_w2 = mtimes(R01.T,pR) - mtimes(R01.T,pL)
#                    g.append(pos_2in1_w1[0:3] - RelPosition[k][0:3]) 
#                    lbg += np.zeros(3).tolist()  
#                    ubg += np.zeros(3).tolist()
#                    RelPosition.append(pos_2in1_w1)
                    #### RELATIVE POSITION ####
...
#                    ubg +=  np.full((1,3),0.001)[0].tolist() 
            ...
            #Fix orientation error between first hand and world
...
#            
            ...
            ##                 COST FUNCTION                 ##   
            ###################################################
            # Se la temperatura è sotto un riferimento, 
            # il mio obiettivo è mantenere la pini,
            # altrimenti voglio raffreddare i motori caldi
            
#            if max(T_0) > 40:
#                for Tem in T_0:
#                    if Tem > 40:
#                        J += 100 * dot(Tem - 40.0, Tem - 40.0)
#            else:       
...
#            opts["qpsol"] = "qpoases"
#            MySolver = "sqpmethod"
#            print("Solving iteration " + str(s) + " con sqp method")
#        else:
#            opts = {"ipopt": {"tol": 0.001,"acceptable_tol": 0.001,"constr_viol_tol" : 0.001, "compl_inf_tol": 0.001}}
#            MySolver = "ipopt"
#            print("Solving iteration " + str(s) + " con IPOPT")
