...
qc_dot0 = np.zeros(nq).tolist() # Joint velocity
...
#Box weight express in GLOBAL FRAME
Fdes = SX([0,0, - 9.81 * m])

#INITIAL CONDITION FOR THE FIRST WARMSTART
F0 = np.array([0, - m*9.81/(2*mu) ,m*9.81/2,0, m*9.81/(2*mu),m*9.81/2]).reshape(1,6)[0].tolist()
sol0 =  np.array(qc_0).reshape(1,14)[0].tolist() + T_0 + qc_dot0 + F0
sol0 = sol0 * N + sol0[0:2*nq]
#Initial guess
initialguess = []        
with open('/home/user/workspace/src/mpc_fatigue/Centauro_solutions/' + folder + 'InitialGuess.csv', 'rb') as solution:
    for line in solution:
        x = line.split(',')
        for val in x:
            initialguess.append(float(val))
...
if Check.OCP == True:
    while s < iterations:    
        ...
        for k in range(N):
            ...
            #Firstly I change the force reference frame in LOCAL
            F_C1robot_environment_local = mtimes(R01.T,F_LR)
            #Change the sign to obtained the force that the environment excert on the robot
            ...
            tau = InvDyn(qc_k, qcd_k, np.zeros(nq).tolist()) + mtimes(JLA.T,W_LA) + mtimes(JRA.T,W_RA)
#            
            g.append(tau)
            lbg += [lbtorque]
            ubg += [ubtorque]
...
#            J += mpc.force_w*dot(F_LR,F_LR)
#            J += mpc.force_w*dot(F_RR,F_RR)
#            J += 0.001*dot(T_k,T_k)
...
#        r = Solver(lbx = lbw, ubx = ubw, lbg = lbg, ubg = ubg)  
        ...
        #Here define the final condition that will be initial for the successive approximation
        ...
        if s == 0:
            with open('/home/user/workspace/src/mpc_fatigue/Centauro_solutions/' + folder + '/InitialGuess.csv', 'wb') as myfile:
...
if Check.OCP == True or Check.Rviz == True:
    ...
    solTOT = []        
    with open('/home/user/workspace/src/mpc_fatigue/Centauro_solutions/' + folder + '/sol_MPCwithfriction.csv', 'rb') as solution:
        for line in solution:
            x = line.split(',')
            for val in x:
    ...
    for i in range(iterations):
        ...
        while (k < N):
            ...
            R10 = np.array(ForwKinLA(sol[k*n:k*n+nq],"rot").T)
            R20 = np.array(ForwKinRA(sol[k*n:k*n+nq],"rot").T)
    ...
    if Check.Rviz == True:
        ...
        ta.talker(np.round(new_qc_opt,4))  
    ...
    lbtq = [];
    #Save the csv file
    SaveCsv(folder,qc_opt,qcd_opt,F_opt_glob,F_opt_loc,tau_LR,tau_RR,lbtq,ubtq,box_pos,box_rpy,Tw,lbTe,ubTe)
